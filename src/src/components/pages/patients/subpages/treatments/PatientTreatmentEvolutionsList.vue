...
import { evolutions } from '/@src/data/patients/treatments/patient-treatments-evolution-list'

type Evolution = typeof evolutions[0]
...
const idSorter: VFlexTableWrapperSortFunction<Evolution> = ({ order, a, b }) => {
  if (order === 'asc') {
...
const evolutionFilter: VFlexTableWrapperFilterFunction<Evolution> = ({ searchTerm, row }) => {
  if (!searchTerm) {
    return true
  }
  ...
  return (
    row.evolution.toLocaleLowerCase().includes(searchTerm.toLocaleLowerCase()) ||
    row.profissional.toLocaleLowerCase().includes(searchTerm.toLocaleLowerCase())
  )
}
...
<template>
  <VFlexTableWrapper :columns="columns" limit="5" :data="data">
    <!--
